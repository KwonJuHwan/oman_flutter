...
class HomeViewModel extends ChangeNotifier {
  ...
  List<int> get selectedIngredientIds => _selectedIngredients.map((e) => e.id).toList(); 
  ...
  Future<void> _loadRecentSearches() async {
    _recentIngredientSearches = await _searchApiService.getRecentSearches('INGREDIENT');
    _recentRecipeSearches = await _searchApiService.getRecentSearches('RECIPE');
    notifyListeners();
  }
  ...
  Future<void> removeRecentSearch(IngredientSimpleDto item) async {
    ...
    try {
      await _searchApiService.deleteRecentSearch(typeStr, item.id, item.name);
    } catch (e) {
  ...
  Future<void> _triggerSearchResult() async {
    ...
    try {
      if (_selectedType == SearchType.recipe) {
        ...
        await _searchApiService.saveRecentSearch("RECIPE", null, recipeName);
        await _loadRecentSearches(); 
       
        final result = await _recipeRepository.getIngredientStatistics(recipeName);
        if (result != null) {
        ...
        final result = await _recipeRepository.getCulinaryRecommendations(selectedIngredientIds);
        ingredientSearchResults = result;
      }
    } catch (e) {
      debugPrint("검색 데이터 로드 에러: $e");
    } finally {
  ...
  void onSearchTextChanged(String text) {
    ...
    _debounce = Timer(const Duration(milliseconds: 300), () async {
      final results = await _recipeRepository.getAutocomplete(text);
  ...
  Future<void> addIngredient(IngredientSimpleDto ingredient) async {
    if (!_selectedIngredients.contains(ingredient)) {
      ...
      // ✨ 최근 검색어 저장: SearchApiService 사용
      await _searchApiService.saveRecentSearch("INGREDIENT", ingredient.id, ingredient.name);
      await _loadRecentSearches(); 
    }
  }
